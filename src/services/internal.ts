...
export function fetchStaffList(data: { current: number; size: number }): Promise<PostData> {
  return axios.post(API.STAFF_LIST, data)
}
export function addStaff(data: { mobile: string; rank: string }): Promise<PostData> {
...
export function updateJob(data: { id: number; param: string }): Promise<PostData> {
